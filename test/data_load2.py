...
with open('../CUB_200_2011/bounding_boxes.txt') as f:
    bounding_boxes = [None] + [list(map(float, line.split()[1:])) for line in f.readlines()]
...
with open('bbox_train.txt', 'w') as f:
    for img_id in train_img_id:
        print(paths[img_id], bounding_boxes[img_id][0], bounding_boxes[img_id][1], bounding_boxes[img_id][2], bounding_boxes[img_id][3], file=f)
